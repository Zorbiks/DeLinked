function injectDebullshitifyToActionBar() {
    ...
    sendAnchors.forEach((sendBtn) => {
        // Find the outermost wrapper for the Send action button
        const actionItemContainer = sendBtn.closest(".feed-shared-social-action-bar__action-button") || sendBtn.closest(".artdeco-hoverable-trigger") || sendBtn;

        // Prevent duplicate insertions
        if (actionItemContainer.nextElementSibling && actionItemContainer.nextElementSibling.classList.contains("delinked-debullshitify-btn")) {
            return;
        }
        ...
        debullshitActionBtn.addEventListener("click", (e) => {
            ...
            const postCard =
                ...
                sendBtn.closest(".feed-shared-update-v2");

            if (!postCard) {
                console.warn("DeLinked: Could not find post card container.");
                return;
            }

            // Locate text container
            const getTextContainer = () =>
                postCard.querySelector(".update-components-update-v2__commentary") ||
                ...
                postCard.querySelector(".feed-shared-inline-show-more-text");
            ...
            const extractText = (container) => {
                ...
                const moreBtnClone = clone.querySelector('[data-testid="expandable-text-button"], .feed-shared-inline-show-more-text__see-more-less-toggle');
        ...
        // Insert into social action bar row directly
        const socialActionBar = sendBtn.closest(".feed-shared-social-action-bar");
        if (socialActionBar) {
            socialActionBar.appendChild(debullshitActionBtn);
        } else {
            actionItemContainer.insertAdjacentElement("afterend", debullshitActionBtn);
        }
    });
}
